...
| 経路 | 起動 | 役割 |
|---|---|---|
| `workflow_dispatch` | cron-job.org が 13:05 JST に起動 | **メイン**。更新直後の最新版が届く |
...
- **Body**: `{"ref":"main"}`
- **実行時刻**: 毎日 13:05 JST（冬時間は APOD の更新が 14:00 JST になりますが、
  取りこぼし経路と `schedule` が拾うため、13:05 のままでも投稿は途切れません）
