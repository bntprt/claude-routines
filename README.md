...
`.github/workflows/apod-daily.yml` は cron `13,33,53 3-14 * * *`（UTC）＝
**12:13〜23:53 JST を 20 分ごと**にチェックし、新しい APOD を見つけた最初の回だけ投稿します。
同じ APOD なら手順 2 のガードで即終了するため、**投稿は 1 日 1 回**です。
空振りの回は NASA API を 1 回叩いて終了するだけで Claude API は呼ばないので、
**課金は実際に投稿する 1 回ぶんだけ**です。

固定時刻を 1 回叩くのではなく多数回チェックにしているのは 2 つの理由からです。

1. 夏時間の切り替わり（13:00 ⇄ 14:00 JST）や公開の遅れに、設定を触らずに追従できる
2. **GitHub Actions の `schedule` は遅延・スキップされることがある**ため。
   実際 2026-08-27 に cron が `0 4-12 * * *` だった頃、9 回の予定がすべてスキップされて
   その日の投稿が飛びました。GitHub は毎時 0 分の混雑を避けるよう案内しているため、
   `:13 / :33 / :53` に散らし、1 日 36 回試すことで多少落とされても通るようにしています。

> **さらに確実にしたい場合**: DHBR と同様に cron-job.org から `workflow_dispatch` を
> 起動する方式を併用できます。外部から叩くため GitHub 側の混雑の影響を受けません。
